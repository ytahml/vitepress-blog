...
先描述出现Mybatis的SQL执行结果与客户端相同SQL执行结果不一致问题的原因, 是因为在调试接口的时候, 参数是从某软件**直接复制**过来在 Apifox 粘贴并发送的;
...
经过多次调试和Debug, 终于在文章: [MyBatis的SQL执行结果和客户端执行结果不一致问题排查](https://blog.csdn.net/modelmd/article/details/128460956) 中找到可能的问题所在;
...
同时可以很显然发现, **虽然 Debug 会将参数值在旁边标出来, 但是不点击打开, 是很难发现隐藏字符的**;
